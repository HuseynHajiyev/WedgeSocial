...
const FooterColumn = ({ title, links } : LinksArrayInterface) => {
  const target = title.toLowerCase() === 'created by' || title.toLowerCase() === 'other projects' ? '_blank' : '_self'
  return (
    <div className='flex flex-col gap-4'>
      <h3 className='text-lg font-semibold'>{title}</h3>
      <ul className='flex flex-col gap-2 text-slate-300'>
        {links.map((link, index) => (
          <Link href={link.href} target={target} key={index} className='text-small'>
            {link.label}
          </Link>
        ))}
      </ul>
